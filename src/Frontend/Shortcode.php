...
?>
    ...
    public function enqueue_scripts()
    {
        ...
        wp_add_inline_script('wp-desa-frontend', 'var wpDesaFrontend={ajaxUrl:"' . esc_url_raw(admin_url('admin-ajax.php')) . '"};', 'before');
    ...
    public function render_layanan($atts = [])
    {
        ...
        // Allow ?jenis=cek to open tracking tab directly (e.g. /wp-desa-layanan/?jenis=cek)
        if (isset($_GET['jenis'])) {
            $jenis = strtolower(sanitize_text_field(wp_unslash($_GET['jenis'])));
            if (in_array($jenis, ['cek', 'cek-status', 'tracking', 'track', 'status', 'lacak'], true)) {
                $view = 'tracking';
            } elseif (in_array($jenis, ['form', 'formulir', 'request', 'ajukan'], true)) {
                $view = 'request';
            }
        }
        // Allow ?kode_tracking=LTR-XXXXX to open the tracking tab and render
        // the result server-side (plain HTML GET, no AJAX).
        $track_code   = '';
        $track_letter = null;
        $track_error  = '';
        if (isset($_GET['kode_tracking'])) {
            $track_code = strtoupper(sanitize_text_field(wp_unslash($_GET['kode_tracking'])));
            if ($track_code !== '') {
                $view = 'tracking';
                ...
                $track_letter  = $wpdb->get_row($wpdb->prepare(
                    "SELECT l.*, t.name as type_name FROM $table_letters l LEFT JOIN $table_types t ON l.letter_type_id = t.id WHERE UPPER(l.tracking_code) = %s",
                    $track_code
                ));
                if (!$track_letter) {
                    $track_error = 'Permohonan dengan kode ' . $track_code . ' tidak ditemukan.';
                }
            }
        }
    ...
    ?>
        <div id="<?php echo esc_attr($uid); ?>" class="wp-desa-wrapper wp-desa-layanan--<?php echo esc_attr($style); ?>"
            ...
            <div class="wp-desa-tab-panel" x-show="tab === 'request'"
                ...
                <form style="<?php echo $form_style; ?>">
                    <div style="padding:<?php echo $is_classic ? 'var(--sp-xl)' : '0'; ?>;">
                        ...
                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Jenis Kelamin</label>
                            <select x-model="form.jenis_kelamin" name="jenis_kelamin" class="wp-desa-select" style="<?php echo $input_style; ?>">
                        ...
                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Tempat Lahir</label>
                            <input type="text" x-model="form.tempat_lahir" name="tempat_lahir" class="wp-desa-input"
                                placeholder="Contoh: Boyolali" style="<?php echo $input_style; ?>">
                        </div>

                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Tanggal Lahir</label>
                            <input type="date" x-model="form.tanggal_lahir" name="tanggal_lahir" class="wp-desa-input"
                                style="<?php echo $input_style; ?>">
                        </div>

                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Warganegara</label>
                            <input type="text" x-model="form.warganegara" name="warganegara" class="wp-desa-input"
                                placeholder="Indonesia" value="Indonesia" style="<?php echo $input_style; ?>">
                        </div>

                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Agama</label>
                            <select x-model="form.agama" name="agama" class="wp-desa-select" style="<?php echo $input_style; ?>">
                        ...
                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Pekerjaan</label>
                            <input type="text" x-model="form.pekerjaan" name="pekerjaan" class="wp-desa-input"
                                placeholder="Contoh: Pelajar/Mahasiswa" style="<?php echo $input_style; ?>">
                        </div>

                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Status Perkawinan</label>
                            <select x-model="form.status_perkawinan" name="status_perkawinan" class="wp-desa-select" style="<?php echo $input_style; ?>">
                        ...
                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            style="<?php echo $is_minimal ? 'margin-bottom:var(--sp-md);' : ''; ?>">
                            <label class="wp-desa-label" style="<?php echo $label_style; ?>">Alamat Sesuai KTP</label>
                            <textarea x-model="form.alamat" name="alamat" class="wp-desa-textarea" rows="2"
                                placeholder="Jalan, RT/RW, Dusun..." style="<?php echo $input_style; ?>"></textarea>
                        </div>
                        ...
                        <div class="wp-desa-form-group <?php echo $gap_class; ?>"
                            ...
                            <small class="wp-desa-helper wp-desa-layanan-type-berlaku"></small>
                        </div>
            ...
            <div class="wp-desa-tab-panel" x-show="tab === 'tracking'" style="<?php echo $track_hide; ?>">
                <form method="get" action="<?php echo esc_url(get_permalink()); ?>#tab-tracking"
                    style="<?php echo $form_style; ?>">
                    <?php if (!get_option('permalink_structure')): ?>
                        <input type="hidden" name="page_id" value="<?php echo esc_attr(get_the_ID()); ?>">
                    <?php endif; ?>
                    <div style="padding:<?php echo $is_classic ? 'var(--sp-xl)' : '0'; ?>;">
                        <div class="wp-desa-form-group">
                            <label class="wp-desa-label" for="<?php echo esc_attr($uid); ?>-kode-tracking">Masukkan Kode Tracking</label>
                            <div class="wp-desa-tracking-input-group" style="display:flex;gap:var(--sp-xs);">
                                <input type="text" name="kode_tracking" id="<?php echo esc_attr($uid); ?>-kode-tracking"
                                    class="wp-desa-input" placeholder="Contoh: LTR-..."
                                    value="<?php echo esc_attr($track_code); ?>" style="flex:1;">
                                <button type="submit" class="<?php echo $btn_class; ?>"
                                    style="font-size:14px;font-weight:600;letter-spacing:0.7px;text-transform:uppercase;">
                ...
                <?php if ($track_letter): ?>
                    <?php
                    $status          = isset($track_letter->status) ? $track_letter->status : '';
                    $status_labels   = [
                        'pending'   => 'Menunggu',
                        'processed' => 'Diproses',
                        'ready'     => 'Siap Diambil',
                    ...
                    $status_style    = isset($status_styles[$status]) ? $status_styles[$status] : '';
                    $created_at      = isset($track_letter->created_at) ? $track_letter->created_at : '';
                    $created_display = $created_at ? date_i18n('j F Y', strtotime($created_at)) : '-';
                    ?>
                    <div class="wp-desa-result-card" style="margin-top: var(--sp-lg);">
                        <div class="wp-desa-card-row">
                            <span class="wp-desa-card-label">Nama Pengaju</span>
                            <span class="wp-desa-card-value"><?php echo esc_html($track_letter->name); ?></span>
                        </div>
                        <div class="wp-desa-card-row">
                            <span class="wp-desa-card-label">Tanggal</span>
                            <span class="wp-desa-card-value"><?php echo esc_html($created_display); ?></span>
                        </div>
                        <div class="wp-desa-card-row">
                            <span class="wp-desa-card-label">Status</span>
                            <span class="wp-desa-status-badge"
                                style="<?php echo esc_attr($status_style); ?>"><?php echo esc_html($status_label); ?></span>
                        </div>
                    </div>
                <?php elseif ($track_error): ?>
                    <div class="wp-desa-error-box"><?php echo esc_html($track_error); ?></div>
                <?php endif; ?>
            </div>
